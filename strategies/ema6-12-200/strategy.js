...
module.exports = {
    ...
    onCandle: function(bot) {
        ...
        if(bot.enoughCandles){
            ...
            console.log("EMA 21: " + bot.actualCandle.sma12)
            console.log("EMA 21: " + bot.actualCandle.sma200)
            console.log("\n")


            // Histograma mayor que 0 y macd > signal  (1D)
            if (!bot.lookback[0] || bot.lookback[0].sma200 == undefined){
                return; 
            } 

            if(bot.state === 'initial' || bot.state === 'sell') {

                if( !(bot.actualCandle.sma200 > bot.lookback[0].sma200) ) {
                    return;
                }
                if(bot.actualCandle.ema6 > bot.actualCandle.sma200){
            ...
            else if(bot.state === 'buy' && bot.actualCandle.ema6 < bot.actualCandle.sma12){
