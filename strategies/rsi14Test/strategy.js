...
module.exports = {
    ...
    onCandle: function(bot, paperTrading) {
        ...
        if(bot.enoughCandles){
            ...
            console.log("RSI 14: " + bot.actualCandle.rsi14)
            console.log("EMA 50: " + bot.actualCandle.ema38 + "\n")


            // Histograma mayor que 0 y macd > signal  (1D)
            if (!bot.lookback[0] || bot.lookback[0].ema38 == undefined){
                return; 
            } 

            if(bot.state === 'initial' || bot.state === 'sell') {

                if(bot.actualCandle.rsi14 > 70){
            ...
            else if(bot.state === 'buy' && bot.actualCandle.ema38 > bot.actualCandle.close){
